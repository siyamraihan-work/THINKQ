...
### Admin analytics
- Open `/analytics/tickets/dashboard`
...
- Returns ordered ticket rows with:
  ...
  - wait time
  - completion time
- CSV export available at `/analytics/tickets/dashboard.csv`
...
- `http://localhost/analytics/tickets/dashboard`
...
- `POST /analytics/tickets/dashboard/export` creates one Excel workbook per location, packages them into a single ZIP response, and then purges the exported tickets from the database.
